    ...
    public function index ()
    {
        if (array_key_exists ('type', $_SESSION))
        {
          if (($_SESSION['type']=== 'admin')||($_SESSION['type']=== 'technician')||($_SESSION['type']=== 'superadmin'))
          {
            ...
            $db_data['readNotifs'] = $this->nm->getReadNotifs ($_SESSION['username'], $_SESSION['type']);
            $db_data['unreadNotifs'] = $this->nm->getUnreadNotifs ($_SESSION['username'], $_SESSION['type']);
            $this->load->view ('Notifications_view', $db_data);
          }
          else if ($_SESSION['type']=== 'client')
          {
            $db_data['unread'] = $this->nm->getUnreadCount ($_SESSION['username'], $_SESSION['type']);
            $db_data['unreadNotifs'] = $this->nm->getClientUnreadNotifs ($_SESSION['username'], $_SESSION['type']);
            $db_data['readNotifs'] = $this->nm->getReadNotifs ($_SESSION['username'], $_SESSION['type']);
            $this->load->view ('Notifications_view', $db_data);
      		}
        }
