<?php
//this is the controller for the API that gets the processed job requests table
if ( ! defined('BASEPATH')) exit('No direct script access allowed');

class Get_processed_job_requests_controller extends CI_Controller
{
    ...
    //if it does it means that there is a user currently in session
      ...
      //if the user in session is a client
			if($_SESSION["type"] === "client"){
				...
				//queries the DB
				$rows1 = $this->jrm->getJobRequestsArray(" WHERE jobStatus='PROCESSED' AND clientID=".$clientID." ");
        ...
        //returns a message instead of a table
				if($nRows1 == 0){
					echo "<h5 class=\"center-align\">Sorry, there are no processed job requests filed under your name at the moment.</h5>";
					return;
				}
        ...
        //queries the database
				$rows1 = $this->jrm->getJobRequestsArray(" WHERE jobStatus='PROCESSED'");
        ...
        //returns a message instead of a table
				if($nRows1 == 0){
					echo "<h5 class=\"center-align\">Sorry, there are no processed job requests at the moment.</h5>";
					return;
				}
