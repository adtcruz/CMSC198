<?php
//this is the controller for the API that gets the table containing search results
if ( ! defined('BASEPATH')) exit('No direct script access allowed');

class Get_search_results_controller extends CI_Controller
{
  public function __construct() {
    ...
    //if it does it means that there is a user currently in session
      ...
      if(array_key_exists("searchKeys",$_POST)){
        ...
        //if the user in session is a client
          ...
          //processes the search string for where clause
          $searchString = $this->jrm->processSearch($_POST["searchKeys"]);

  				//queries the DB
  				$rows1 = $this->jrm->getJobRequestsArray(" WHERE (".$searchString.") AND clientID=".$clientID." ");
          ...
          //queries the database
  				$rows1 = $this->jrm->getJobRequestsArray(" WHERE (".$searchString.") ");
